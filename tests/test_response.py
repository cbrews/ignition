...
class ResponseFactoryTests(TestCase):
    def test_creates_input_response(self):
        response_object1 = ResponseFactory.create(
            "url", "10", meta="Some input", raw_body=b""
        )
    ...
    def test_creates_success_response(self):
        response_object = ResponseFactory.create(
            "url", "20", meta="text/gemini", raw_body=b"Some body"
        )
...
class TempFailureResponseTests(TestCase):
    ...
    def test_data(self):
        self.assertEqual(
            self.response.data(),
            "40 The server had trouble processing your response. Please try again.",
        )
...
class PermFailureResponseTests(TestCase):
    ...
    def test_meta(self):
        self.assertEqual(
            self.response.meta, "There was a permanent error on this page."
        )
    ...
    def test_data(self):
        self.assertEqual(
            self.response.data(), "50 There was a permanent error on this page."
        )
...
class ClientCertRequiredResponseTests(TestCase):
    ...
    def test_basic_status(self):
        self.assertEqual(
            self.response.basic_status, RESPONSE_STATUS_CLIENTCERT_REQUIRED
        )

    def test_status(self):
        self.assertEqual(
            self.response.status, RESPONSE_STATUSDETAIL_CLIENTCERT_REQUIRED
        )

    def test_meta(self):
        self.assertEqual(
            self.response.meta, "Please create a client certificate for this request."
        )
    ...
    def test_data(self):
        self.assertEqual(
            self.response.data(), "Please create a client certificate for this request."
        )
...
class ErrorResponseUnknownStatusTests(TestCase):
    ...
    def setUp(self):
        self.response = ResponseFactory.create(
            "gemini://test.com/", "99", meta="THIS IS A BAD RESPONSE"
        )
    ...
    def test_meta(self):
        self.assertEqual(
            self.response.meta,
            "Invalid response received from the server, status code: 99",
        )
